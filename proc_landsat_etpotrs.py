...
# Define Info Message
overview = """This module is a ET potential radiative method claculation for Landsat 7. It corrects every band (1,2,3,4,5,6L,6H,7,8Pan) from DN to either reflectance at top of Atmosphere or Kinetic Temperature. Then it calculates the necessary things for a potential ET according to diurnal incoming solar radiation. 
...
output=string //ETpot output layer name"""

class MyFrame(wx.Frame):
	def __init__(self,parent, id=-1, title='ETPOT for Landsat image from .met metadata file',
			pos=(0,0),
			size=(500,400),
		...
		# Initialize RS.py functions
		self.pre=rs.pre()
		self.albedo=rs.albedo()
		self.emissivity=rs.emissivity()
		self.vi=rs.vi()
		self.et_pot_rs=rs.et_pot_rs()
		self.eb=rs.eb()
		self.utils=rs.utils()
		# Initialize metadata
	...
	def OnOK(self,event):
		...
		if(self.inputFile==''):
			...
			self.metadata = self.pre.read_met_file_landsat7(self.inputFile)
		# Process Thread 1
		self.inband = gdal.Open( self.metadata[0], GA_ReadOnly )
		self.Lmin = self.metadata[9]
		self.LMax = self.metadata[18]
		self.Qcalmin = self.metadata[27]
		self.QcalMax = self.metadata[36]
		#self.inband = self.band1.GetRasterBand(1)
		#create copy of input array for results
		self.result1 = 1.0*self.inband
		for self.y in range(inband.YSize - 1, -1, -1):
			self.scanline = self.inband.ReadAsArray(0, self.y, self.inband.XSize, 1, self.inband.XSize, 1)
			for self.x in range(0, inband.XSize - 1, 1):
				self.pixel = self.scanline[0][self.x]
				self.pixel = self.pre.dn2rad_landsat7( self.Lmin, self.LMax, self.QcalMax, self.Qcalmin, self.pixel )
				self.scanline[0][self.x] = self.pixel
			self.result1.WriteArray(scanline, 0, self.y)

		del self.band1 #Free Memory
		self.doy = self.utils.date2doy(self.metadata[50],self.metadata[49],self.metadata[48])
		self.kexo = self.pre.kexo(self.metadata[52],self.metadata[51],"band1")
		self.result1 = self.pre.rad2ref_landsat7( self.result1, self.doy, self.metadata[46], self.k_exo )
		# End of Process Thread 1
		# Process Thread 2
		self.band2 = gdal.Open(self.metadata[1], GA_ReadOnly)
		self.Lmin = self.metadata[10]
		self.LMax = self.metadata[19]
		self.Qcalmin = self.metadata[28]
		self.QcalMax = self.metadata[37]
		self.result2 = self.pre.dn2rad_landsat7( self.Lmin, self.LMax, self.QcalMax, self.Qcalmin, self.band2 )
		del self.band2 #Free Memory
		self.doy = self.utils.date2doy(self.metadata[50],self.metadata[49],self.metadata[48])
		self.kexo = self.pre.kexo(self.metadata[52],self.metadata[51],"band2")
		self.result2 = self.pre.rad2ref_landsat7( self.result2, self.doy, self.metadata[46], self.k_exo )
		# End of Process Thread 2
		# Process Thread 3
		self.band3 = gdal.Open(self.metadata[2], GA_ReadOnly)
		self.Lmin = self.metadata[11]
		self.LMax = self.metadata[20]
		self.Qcalmin = self.metadata[29]
		self.QcalMax = self.metadata[38]
		self.result3 = self.pre.dn2rad_landsat7( self.Lmin, self.LMax, self.QcalMax, self.Qcalmin, self.band3 )
		del self.band3 #Free Memory
		self.doy = self.utils.date2doy(self.metadata[50],self.metadata[49],self.metadata[48])
		self.kexo = self.pre.kexo(self.metadata[52],self.metadata[51],"band3")
		self.result3 = self.pre.rad2ref_landsat7( self.result3, self.doy, self.metadata[46], self.k_exo )
		# End of Process Thread 3
		# Process Thread 4
		self.band4 = gdal.Open(self.metadata[3], GA_ReadOnly)
		self.Lmin = self.metadata[12]
		self.LMax = self.metadata[21]
		self.Qcalmin = self.metadata[30]
		self.QcalMax = self.metadata[39]
		self.result4 = self.pre.dn2rad_landsat7( self.Lmin, self.LMax, self.QcalMax, self.Qcalmin, self.band4 )
		del self.band4 #Free Memory
		self.doy = self.utils.date2doy(self.metadata[50],self.metadata[49],self.metadata[48])
		self.kexo = self.pre.kexo(self.metadata[52],self.metadata[51],"band4")
		self.result4 = self.pre.rad2ref_landsat7( self.result4, self.doy, self.metadata[46], self.k_exo )
		# End of Process Thread 4
		# Process Thread 5
		self.band5 = gdal.Open(self.metadata[4], GA_ReadOnly)
		self.Lmin = self.metadata[13]
		self.LMax = self.metadata[22]
		self.Qcalmin = self.metadata[31]
		self.QcalMax = self.metadata[40]
		self.result5 = self.pre.dn2rad_landsat7( self.Lmin, self.LMax, self.QcalMax, self.Qcalmin, self.band5 )
		del self.band5 #Free Memory
		self.doy = self.utils.date2doy(self.metadata[50],self.metadata[49],self.metadata[48])
		self.kexo = self.pre.kexo(self.metadata[52],self.metadata[51],"band5")
		self.result5 = self.pre.rad2ref_landsat7( self.result5, self.doy, self.metadata[46], self.k_exo )
		# End of Process Thread 5
		# Process Thread 6
		if(chosen_tempk_band=='Band61'):
			self.band61 = gdal.Open(self.metadata[5], GA_ReadOnly)
			self.Lmin = self.metadata[14]
			self.LMax = self.metadata[23]
			self.Qcalmin = self.metadata[32]
			self.QcalMax = self.metadata[41]
			self.result6 = self.pre.tempk_landsat7( self.band61 )
			del self.band61 #Free Memory
		else:
			# Process Thread 7
			self.band62 = gdal.Open(self.metadata[6], GA_ReadOnly)
			self.Lmin = self.metadata[15]
			self.LMax = self.metadata[24]
			self.Qcalmin = self.metadata[33]
			self.QcalMax = self.metadata[42]
			self.result6 = self.pre.tempk_landsat7( self.band62 )
			del self.band62 #Free Memory
		# End of Process Thread 6
		# Process Thread 7
		self.band7 = gdal.Open(self.metadata[7], GA_ReadOnly)
		self.Lmin = self.metadata[16]
		self.LMax = self.metadata[25]
		self.Qcalmin = self.metadata[34]
		self.QcalMax = self.metadata[43]
		self.result7 = self.pre.dn2rad_landsat7( self.Lmin, self.LMax, self.QcalMax, self.Qcalmin, self.band7 )
		del self.band7 #Free Memory
		self.doy = self.utils.date2doy(self.metadata[50],self.metadata[49],self.metadata[48])
		self.kexo = self.pre.kexo(self.metadata[52],self.metadata[51],"band7")
		self.result7 = self.pre.rad2ref_landsat7( self.result7, self.doy, self.metadata[46], self.k_exo )
		# End of Process Thread 7
		
		self.albedo=self.pre.albedo_landsat( self.result1, self.result2, self.result3, self.result4, self.result5, self.result7 )
		del self.result1 #Free Memory
		del self.result2 #Free Memory
		del self.result5 #Free Memory
		del self.result7 #Free Memory
		self.ndvi=self.vi.ndvi(self.result3,self.result4)
		...
		self.tsw=0.7
		self.roh_w=1005.0
		self.solar_day=self.et_pot_rs.solarday(self.latitude,self.doy,self.tsw)
		self.rnet_day=self.et_pot_rs.rnetday(self.albedo,self.solar_day,self.tsw)
		self.etpot=self.et_pot_rs.etpotday(self.albedo,self.solarday,self.result6,self.tsw,self.roh_w)
		self.out = self.output+"etpotrs"
		SaveArrayWithGeo(self.etpot,self.band7,self.out,'GTiff')
		self.Destroy()
	...
	def fbbCallback1(self, evt):
		...
		# Front text
	def make_text(self):
		self.text = wx.StaticText(self, -1, "ET POT Landsat7ETM+ using gdal and numeric")
